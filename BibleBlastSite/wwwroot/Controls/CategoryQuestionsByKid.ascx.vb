...
Partial Class Controls_CategoryQuestionsByKid
    ...
    Private Sub AnswerQuestion(ByVal KidID As Integer, ByVal QuestionID As String)
        Dim conn As New Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("BBData").ConnectionString)
        Dim myTimeZone As TimeZoneInfo = TimeZoneInfo.FindSystemTimeZoneById("Central Standard Time")

        Dim todayDate As Date
        todayDate = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, myTimeZone)

        Dim inscom As String = "INSERT INTO QuestionAnswers(KidID,QuestionID,Date,SubmittedBy) VALUES (" & KidID & "," & QuestionID & ",'" & todayDate & "','" & My.User.Name.ToString & "');"
